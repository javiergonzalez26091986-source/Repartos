...
# NUEVA VARIABLE DE CONTROL DE PROCESO
...
with head_r:
    st.write("##") 
    if st.button("🏁 FINALIZAR ENTREGAS", type="primary", width="stretch"):
...
if st.session_state.get('confirmar_cierre'):
    ...
    if cc1.button("❌ NO, VOLVER", width="stretch"):
        st.session_state.confirmar_cierre = False
        st.rerun()
    if cc2.button("🚨 SÍ, CERRAR", width="stretch", type="primary"):
...
if st.session_state.cedula and st.session_state.nombre:
    if not st.session_state.hora_ref or st.session_state.hora_ref in ["", "None"]:
        ...
        if st.button("▶️ INICIAR ENTREGAS", width="stretch"):
        ...
        if info:
            ...
            if not st.session_state.bloqueo_envio:
                if st.button("ENVIAR REGISTRO ✅", width="stretch", type="primary"):
                    if cant_final <= 0:
                        st.warning("La cantidad debe ser mayor a 0")
                    else:
                        st.session_state.bloqueo_envio = True
                        st.rerun() # Recarga para ocultar el botón e iniciar el proceso
            else:
                ...
                st.session_state.bloqueo_envio = False # Se libera el bloqueo para el siguiente registro
