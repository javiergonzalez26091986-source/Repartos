...
# --- BLOQUE DE SEGURIDAD PARA OCULTAR GITHUB Y MENÚ ---
st.markdown("""
    <style>
...
# --- PERSISTENCIA POR URL (RECUPERACIÓN AL INICIAR) ---
...
# Identificación
c1, c2 = st.columns(2)
# Usamos el value de la session_state para que NO se borre al refrescar
...
# Si el usuario escribe, guardamos de inmediato en la sesión y en la URL
...
if st.session_state.cedula and st.session_state.nombre:
    ...
    if not st.session_state.hora_ref or st.session_state.hora_ref in ["", "None"]:
        ...
        if st.button("▶️ CAPTURAR HORA DE SALIDA", use_container_width=True):
            ...
            # Actualizamos la URL con los 3 datos: Ced, Nom y Hor
        ...
        if info:
            ...
            if st.button("ENVIAR REGISTRO ✅", use_container_width=True, type="primary"):
                ...
                # NUEVA HORA BASE -> Se actualiza en la URL para no perder el hilo
