...
st.markdown("""
    ...
    .stButton > button[key="btn_top_final"] {
...
# --- PERSISTENCIA ---
js_get_data = """(async () => { return { ced: localStorage.getItem('sergem_ced'), nom: localStorage.getItem('sergem_nom'), hor: localStorage.getItem('sergem_hor') }; })()"""
...
with head_r:
    ...
    if st.button("🏁 FINALIZAR", type="primary", key="btn_top_final", use_container_width=True):
        st.session_state.confirmar_cierre = True

if st.session_state.get('confirmar_cierre'):
    st.error("⚠️ ¿Cerrar sesión y reiniciar todo?")
    c1, c2 = st.columns(2)
    if c1.button("❌ NO", use_container_width=True):
        st.session_state.confirmar_cierre = False
        st.rerun()
    if c2.button("🚨 SÍ, SALIR", use_container_width=True):
...
if st.session_state.cedula and st.session_state.nombre:
    if not st.session_state.hora_ref or st.session_state.hora_ref in ["", "None"]:
        ...
        with f2: 
            ops_p = ["PANADERIA"] if ciudad == "MANIZALES" else (["POLLOS"] if ciudad in ["MEDELLIN", "BOGOTA"] else ["POLLOS", "PANADERIA"])
            producto = st.radio("📦 Producto:", ops_p, horizontal=True, key="s_prod")
        ...
        if ciudad != "--" and empresa != "--":
            if empresa == "CAÑAVERAL":
                p1, p2 = st.columns(2)
                co = p1.selectbox("📦 Origen:", ["--"] + sorted(LISTA_CANAVERAL), key="co")
                cd = p2.selectbox("🏠 Destino:", ["--"] + sorted(LISTA_CANAVERAL), key="cd")
                ...
                if producto == "PANADERIA":
                    p1, p2 = st.columns(2)
                    t_o = p1.selectbox("📦 Recoge:", ["--"] + sorted(list(dic.keys())), key="to")
                    t_d = p2.selectbox("🏠 Entrega:", ["--"] + sorted(list(dic.keys())), key="td")
                    if t_o != "--" and t_d != "--": info = {"TO": t_o, "CO": dic[t_o], "TD": t_d, "CD": dic[t_d]}
                else:
                    t_sel = st.selectbox("🏪 Tienda:", ["--"] + sorted(list(dic.keys())), key="ct")
        ...
        if info:
            ...
            if st.button("ENVIAR REGISTRO ✅", use_container_width=True, type="primary"):
                ...
                try: requests.post(URL_GOOGLE_SCRIPT, json=payload, timeout=10)
                except: pass
                st.session_state.hora_ref = h_llegada
                actualizar_url_y_disco()
                for k in ['s_ciu', 's_emp', 'co', 'cd', 'ct', 'to', 'td', 'ccant']: 
                    if k in st.session_state: del st.session_state[k]
                st.success(f"Enviado. Nueva base: {h_llegada}")
                time.sleep(1)
                st.rerun()
