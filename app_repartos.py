...
# --- BLOQUE DE SEGURIDAD ABSOLUTA Y ESTILOS ---
st.markdown("""
    <style>
    ...
    .stButton > button[key="btn_finalizar_top"] {
...
if st.session_state.cedula and st.session_state.nombre:
    ...
    if not st.session_state.hora_ref or st.session_state.hora_ref in ["", "None"]:
        ...
        # Selectores
        f1, f2 = st.columns(2)
        ciudad = f1.selectbox("📍 Ciudad:", ["--", "CALI", "MANIZALES", "MEDELLIN", "BOGOTA"], key="s_ciu")
        
        ops_prod = ["POLLOS", "PANADERIA"]
        if ciudad == "MANIZALES": ops_prod = ["PANADERIA"]
        elif ciudad in ["MEDELLIN", "BOGOTA"]: ops_prod = ["POLLOS"]
        producto = f2.radio("📦 Producto:", ops_prod, horizontal=True, key="s_prod")
        
        ops_emp = ["--", "EXITO-CARULLA-SURTIMAX-SUPERINTER"]
        if ciudad == "CALI": ops_emp.append("CAÑAVERAL")
        empresa = st.selectbox("🏢 Empresa:", ops_emp, key="s_emp")

        info = None
        if ciudad != "--" and empresa != "--":
            if empresa == "CAÑAVERAL":
                c1c, c2c = st.columns(2)
                co = c1c.selectbox("📦 Origen:", ["--"] + sorted(LISTA_CANAVERAL), key="co")
                cd = c2c.selectbox("🏠 Destino:", ["--"] + sorted(LISTA_CANAVERAL), key="cd")
        ...
        # El botón solo aparece si info está completo
        if info:
