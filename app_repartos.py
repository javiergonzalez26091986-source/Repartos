...
# --- BLOQUE DE SEGURIDAD Y ADVERTENCIA PERSONALIZADA ---
st.markdown("""
    <style>
    ...
    /* Estilo para un banner de advertencia visual */
    .banner-urgente {
...
# JavaScript para intentar forzar el mensaje y detectar salida
# Nota: Si el navegador bloquea el texto personalizado, el banner rojo compensa visualmente.
html_code = """
<script>
window.onbeforeunload = function(e) {
    var msg = "¡ATENCIÓN! Los datos se perderán si cierra la ventana y deberá llamar al administrador.";
    e.returnValue = msg;
    return msg;
};
</script>
...
if "ced" in params and "cedula" not in st.session_state: st.session_state.cedula = params["ced"]
if "nom" in params and "nombre" not in st.session_state: st.session_state.nombre = params["nom"]
if "hor" in params and "hora_ref" not in st.session_state: st.session_state.hora_ref = params["hor"]
...
# Banner de advertencia constante si hay una sesión activa
if st.session_state.hora_ref:
    st.markdown("""
        <div class="banner-urgente">
            ⚠️ PROHIBIDO CERRAR LA VENTANA: Los datos se perderán y deberá llamar al administrador.
        </div>
    """, unsafe_allow_html=True)

with st.sidebar:
    if st.button("🏁 FINALIZAR DÍA", type="primary"):
        # Al finalizar el día, quitamos la alerta de JavaScript para que pueda cerrar
        html_clear = "<script>window.onbeforeunload = null;</script>"
        html(html_clear, height=0)
