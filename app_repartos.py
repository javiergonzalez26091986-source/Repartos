...
# --- PERSISTENCIA AUTOMÁTICA POR URL ---
...
# Si cambian datos de ID, se actualiza la URL automáticamente
...
if st.session_state.cedula and st.session_state.nombre:
    ...
    # MOSTRAR EL LINK DE RECUPERACIÓN SIEMPRE QUE HAYA UNA HORA ACTIVA
    if st.session_state.hora_ref:
        nom_encoded = quote(st.session_state.nombre)
        # Cambia esto por tu URL real
        link_rec = f"https://repartos-sergem.streamlit.app/?ced={st.session_state.cedula}&nom={nom_encoded}&hor={st.session_state.hora_ref}"
        
        st.markdown(f"""
            <div class="link-info">
                <b>🔗 Link de recuperación:</b><br>
                <code style="word-break: break-all;">{link_rec}</code><br>
                <small>Si cierras el navegador, abre este link para continuar donde ibas.</small>
            </div>
        """, unsafe_allow_html=True)
        ...
        if info:
            ...
            if st.button("ENVIAR REGISTRO ✅", use_container_width=True, type="primary"):
                ...
                payload = {
                    ...
                    "Tienda_O": info["TO"], "Cod_O": info["CO"], "Tienda_D": info["TD"], "Cod_D": info["CD"],
                ...
                # ACTUALIZACIÓN AUTOMÁTICA DE HORA BASE EN LA URL
